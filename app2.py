...
class GoodDeedApp(ctk.CTk):
    ...
    def _update_challenge_text(self, challenge_text):
        """Updates the info label on the camera page with the current challenge."""
        if challenge_text:
    ...
    def _handle_login_trigger(self, vision_data):
        log.info(f"APP: _handle_login_trigger called with vision_data: {vision_data}")
        
        zone = vision_data["zone"]
        user = vision_data["user_data"]
        encoding = vision_data["encoding"]
